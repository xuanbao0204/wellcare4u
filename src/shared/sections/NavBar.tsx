...
const NavBar = () => {
    ...
    const handleLogout = async () => {
        const ok = await confirm({ title: "Xác nhận đăng xuất", message: "Bạn có chắc chắn muốn đăng xuất?" });
        if (!ok) {
            return;
        }
        logout();
    ...
    return (
        <nav className="absolute top-0 z-50 w-full px-4 py-4 no-print">
            <div className="mx-auto flex w-full max-w-7xl items-center justify-between rounded-2xl border border-white/60 bg-white/75 px-5 py-3 text-foreground shadow-[0_12px_35px_-24px_rgba(3,0,43,0.55)] backdrop-blur-xl">
                ...
                <div className="flex min-w-44 items-center justify-end gap-3">
                    ...
                    <div className="relative" ref={notiWrapperRef}>
                        ...
                        {openNoti && (
                            <div className="absolute right-0 mt-3 w-100 overflow-hidden rounded-[28px] border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,1),rgba(248,250,252,1))] shadow-[0_22px_55px_-24px_rgba(3,0,43,0.5)] backdrop-blur-xl animate-in fade-in zoom-in-95">
                                    ...
                                    {notifications.length === 0 ? (
                                        ...
                                        notifications.map((n) => (
                                            <div
                                                ...
                                                className={`group rounded-[22px] border px-3 py-3.5 transition my-1 ${n.isRead
                                ...
                                <div className="cursor-pointer border-t border-primary/10 bg-white/80 px-4 py-2.5 text-center text-sm font-semibold text-primary transition hover:bg-primary/5">
                                    <button onClick={() => router.push(`/${user?.role.toLowerCase()}/notifications`)}>Xem tất cả</button>
                                </div>
                            </div>
                        )}
                    </div>
